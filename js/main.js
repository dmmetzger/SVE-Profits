...
var miniBar = 8;
var barOffsetX = 48; /* Offset for bar graphs from axis - default value 29 */
var barOffsetY = 40;
...
function fetchCrops() {
	...
	for (var i = 0; i < season.crops.length; i++) {
	    if ((options.seeds.pierre && season.crops[i].seeds.pierre != 0) ||
			...
			(options.seeds.sve && season.crops[i].seeds.sveLoc != "") ||
		...
		.append("rect")
			...
			.on("mouseover", function(d) {
				...
				if (options.extra) {
					...
					if (d.seeds.sve >= 0 && d.seeds.pierre == 0 && d.seeds.joja == 0 && d.seeds.special == 0) { // SVE Special seeds are for seeds designated as unpurchasable
						tooltipTr = tooltipTable.append("tr");
						if (first) {
